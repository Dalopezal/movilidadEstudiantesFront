...
export class CondicionConvocatoriaComponent implements OnInit, OnDestroy {
    ...
    this.api.get<any>('CondicionConvocatoria/Consultar_CondicionesConvocatoria')
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.error = this.translate.instant('CONDICION_CONVOCATORIA.MENSAJES.ERROR_CARGA');
          this.data = [];
          this.filteredData = [];
          this.pagedData = [];
          this.calculateTotalPages();
          this.showError(this.translate.instant('CONDICION_CONVOCATORIA.MENSAJES.ERROR_CARGA'));
          this.loadingTable = false;
        }
      });
  }
  ...
  filterRelaciones() {
    ...
    if (!this.filtro || this.filtro.trim() === '') {
      this.showWarning(this.translate.instant('CONDICION_CONVOCATORIA.MENSAJES.FILTRO_VACIO'));
      return;
    }
    this.loadingTable = true;
  ...
  onSubmit(form: NgForm) {
    ...
    if (!this.model.condicionesId || !this.model.convocatoriaId) {
      this.error = this.translate.instant('CONDICION_CONVOCATORIA.MENSAJES.CAMPOS_OBLIGATORIOS');
      return;
    }
    ...
    obs.pipe(takeUntil(this.destroy$)).subscribe({
      next: (response) => {
        ...
        if (response.exito && response.datos) {
          ...
          this.showError(this.translate.instant('CONDICION_CONVOCATORIA.TOASTS.ERROR_DESC'));
        }
      },
      error: (err) => {
  ...
  async deleteItem(condicioId: string, convocatoriaId: string) {
    const confirmado = await this.showConfirm(this.translate.instant('CONDICION_CONVOCATORIA.CONFIRM.ELIMINAR'));
    ...
    this.api.delete(`CondicionConvocatoria/Eliminar_CondicionesConvocatoria/${condicioId},${convocatoriaId}`)
      ...
      .subscribe({
        next: () => {
          this.fetchRelaciones();
          this.showSuccess(this.translate.instant('CONDICION_CONVOCATORIA.TOASTS.EXITO_DESC'));
        },
        error: (err) => {
  ...
  showSuccess(mensaje: any) {
    toast.success(this.translate.instant('CONDICION_CONVOCATORIA.TOASTS.EXITO_TITULO'), {
      description: mensaje,
      unstyled: true,
  ...
  showError(mensaje: any) {
    toast.error(this.translate.instant('CONDICION_CONVOCATORIA.TOASTS.ERROR_TITULO'), {
      description: mensaje,
      unstyled: true,
  ...
  showWarning(mensaje: string) {
    toast.warning(this.translate.instant('CONDICION_CONVOCATORIA.TOASTS.WARNING_TITULO'), {
      description: mensaje,
      unstyled: true,
  ...
  showConfirm(mensaje: string): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      this.confirmationService.confirm({
        message: `¿${mensaje}?`,
        header: this.translate.instant('CONDICION_CONVOCATORIA.CONFIRM.HEADER'),
        icon: 'pi pi-exclamation-triangle custom-confirm-icon',
        acceptLabel: this.translate.instant('CONDICION_CONVOCATORIA.CONFIRM.ACEPTAR'),
        rejectLabel: this.translate.instant('CONDICION_CONVOCATORIA.CONFIRM.CANCELAR'),
